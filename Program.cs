...
//thanks er-v
//> dotnet add package NPOI --version 2.5.1
...
namespace ConvertXlsToXlsx
{
    class Program
    {
        ...
        {
            Console.WriteLine("開始轉檔 : xls to xlsx");

            //init source xls and destination xlsx
            var source = new HSSFWorkbook(new FileStream("file1.xls", FileMode.Open));
            ...
            destination.Write(new FileStream("test.xlsx", FileMode.Create, FileAccess.ReadWrite));//save xlsx

            Console.WriteLine("轉檔成功!!");
            Console.ReadKey();
        }
